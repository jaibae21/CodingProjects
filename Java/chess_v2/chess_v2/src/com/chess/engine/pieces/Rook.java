...
public class Rook extends Piece{
    ...
    private final static int[] CANDIDATE_MOVE_VCOORDS = {-8, -1, 1, 8};

    Rook(int piecePostition, Alliance pieceAlliance) {
    ...
    private static boolean isFirstColExclusion(final int currentPos, final int candidateOffset) {
        return BoardUtils.FIRST_COL[currentPos] && (candidateOffset == -1);
    }

    private static boolean isEighthColExclusion(final int currentPos, final int candidateOffset) {
        return BoardUtils.EIGHT_COL[currentPos] && (candidateOffset == 1);
    }
}
